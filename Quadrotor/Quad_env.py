...
class BaseAviary(BenchmarkEnv):
    ...
    def __init__(self,
       ...
       #self.INIT_XYZS= initial_xyzs
        ...
        self.DRONE_IDS = np.array([p.loadURDF(self.URDF_PATH,self.INIT_XYZS[i, :],p.getQuaternionFromEuler(self.INIT_RPYS[i, :]),flags = p.URDF_USE_INERTIA_FROM_FILE,physicsClientId=self.PYB_CLIENT)
    ...
    def _advance_simulation(self, clipped_action, disturbance_force=None):
        ...
        # Update and store the drones kinematic informatself.step_counter ion.
        self._update_and_store_kinematic_information()
    ...
    def _computeObs(self):
        ...
        return {str(i): {"state": self._get_drone_state_vector(i), "neighbors": adjacency_mat[i, :]} for i in range(self.NUM_DRONES)}
