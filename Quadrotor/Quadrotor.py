...
from benchmark_env2 import Cost, Task
from safe_control_gym.envs.constraints import GENERAL_CONSTRAINTS
from safe_control_gym.math_and_models.symbolic_systems import SymbolicModel
from Quadrotor_utils import QuadType, cmd2pwm, pwm2rpm
...
from base_aviary2 import BaseAviary2
print("quadrotor")
class Quadrotor(BaseAviary2):
    ...
    def render(self, mode='human'):
        ...
        pos = state[[0,2,4]]
        vel = state[[1,3,5]]
        yawpitchroll = state[[6,7,8]]
        yawrate,pitchrate,rollrate = state[[9,10,11]]
       ...
       # Update camera parameters
        self.CAM_DISTANCE = self.CAM_DISTANCE + 0.1*(np.linalg.norm(vel) - self.CAM_DISTANCE) # move closer if quadrotor is moving fast
        self.CAM_YAW = self.CAM_YAW + 0.1*yawrate
        self.CAM_PITCH = self.CAM_PITCH + 0.1*pitchrate
        self.CAM_ROLL = self.CAM_ROLL + 0.1*rollrate
