...
class MPC(BaseController):
    ...
    def __init__(
        ...
        self.startraj_indx = 0
        self.endtraj_indx = self.T+1

        self.goal_states = np.zeros((12,self.T+1))
    ...
    def reset(self):
        ...
        if self.env.TASK == Task.STABILIZATION:
            ...
            self.mode = "tracking"
            self.traj,self.Goals_indx =create_trajec(self.waypoints,self.env.EPISODE_LEN_SEC,self.env.CTRL_FREQ,deg=self.deg,intermediatepoints=self.intermediatepoints) #self.env.X_GOAL.T
            self.env.X_GOAL = self.traj.T
        ...
        self.initial_obs, self.initial_info = self.env.reset()
    ...
    def add_obstacles(self):
        if self.obstacles:
            self.Obs = Obstacles()
            self.Obs.create_static_obstacles(self.env,self.waypoints)
            self.Obs.static_obstacles_info()
            self.Obs.create_dynamic_obstacle(self.env, self.waypoints)
            self.Obs.goals_obstacles_info()
    ...
    def get_goal_obstac_indx(self):
        indx_trajec = self.startraj_indx 
        goal_obs = self.Obs.goals_objects
        goals_indx = self.Goals_indx
        count=len(goals_indx)-1
        object_indx = "false"
        for indx in goals_indx[::-1]:
            if indx_trajec-.75*self.T<=indx:
                object_indx = count
            count-=1    
        return object_indx
    ...
    def setup_optimizer(self):
        ...
        slack_goals = opti.variable(len(self.Obs.goals_obstacles))
        slack_static = opti.variable(len(self.Obs.static_obstacles))
        slack_dynamic = opti.variable(len(self.Obs.dynamic_obstacles))


        # cost (cumulative)
        cost = 0
        cost_func = self.model.loss
        self.Obs.initiator(self.env)
        self.Obs.dynamic_obstacles_info()
        goal_obs_indx = self.get_goal_obstac_indx()
        self.goal_obs_indx = goal_obs_indx
        dyn_weight = 0.004#0.08
        static_weight = 0.004#0.04
        for i in range(T):
        ...
        keys_dyn_obstac = list(self.Obs.dynamic_objects.keys())
        count=0
        for key in keys_dyn_obstac:
            obstac = self.Obs.dynamic_objects[key]
            d_obs = obstac["d_obs"]
            d_robot = obstac["d_robot"]
            x_obs,y_obs,z_obs = obstac["x_center"],obstac["y_center"],obstac["z_center"]
            vel_obsx,vel_obsy,vel_obsz = obstac["x_vel"],obstac["y_vel"],obstac["z_vel"]      
            slack = slack_dynamic[count]      
            for i in range(self.T+1):
                x_obss =x_obs+ self.dt*i*vel_obsx
                y_obss =y_obs + self.dt*i*vel_obsy
                z_obss = z_obs + self.dt*i*vel_obsz

                x_rob,y_rob,z_rob = x_var[0,i],x_var[2,i],x_var[4,i]
                min_distance = (d_robot+d_obs)/2
                distance = cs.sqrt((x_obss - x_rob)**2 + (y_obss - y_rob)**2 + (z_obss - z_rob)**2)
                min_distance = (d_robot+d_obs)/2
                cost += dyn_weight /(distance**2-min_distance**2)
                opti.subject_to(distance >=min_distance+slack)
            count+=1
        ...
        for i in range(self.T+1):
            keys_obstac = list(self.Obs.static_objects.keys())
            count=0
            for key in keys_obstac:
                
                obstac = self.Obs.static_objects[key]
                d_obs = obstac["d_obs"]
                d_robot = obstac["d_robot"]
                x_obs,y_obs,z_obs = obstac["x_center"],obstac["y_center"],obstac["z_center"],
                x_rob,y_rob,z_rob = x_var[0,i],x_var[2,i],x_var[4,i]
                slack = slack_static[count]
                distance = cs.sqrt((x_obs - x_rob)**2 + (y_obs - y_rob)**2 + (z_obs - z_rob)**2)
                min_distance = (d_robot+d_obs)/2

                cost += static_weight /(distance**2-min_distance**2)
                opti.subject_to(distance >=min_distance+slack)
                count+=1
            count=0
            if i ==0:
                ref_traj_to = self.goal_states[[0,2,4],i ]
                
                p.addUserDebugLine(ref_traj_to, [self.env.state[0], self.env.state[2], self.env.state[4]], [1, 1, 0],physicsClientId=self.env.PYB_CLIENT)



            for index,obstac in enumerate(self.Obs.goals_objects):
                slack = slack_goals[count]
                count+=1
                R_torus = self.Obs.R_goal
                d_obs = obstac["d_obs"]
                d_robot = obstac["d_robot"]
                x_obs,y_obs,z_obs = obstac["x_center"],obstac["y_center"],obstac["z_center"]
                center_obs = np.array([x_obs,y_obs,z_obs] ).reshape(3,1)

                R_matrix = self.Obs .goal_rotations[index]
                T_torus_world = self.Obs.Trans_goals[index]


                pointsOntorus_world = self.Obs.point_goals_torus[index]

                front_shifted_goal = np.array([0,0,0.15,1])
                front_shifted_goal =   (T_torus_world@front_shifted_goal)[:3].tolist()
                x_obss,y_obss,z_obss = front_shifted_goal[0],front_shifted_goal[1],front_shifted_goal[2]


                back_shifted_goal = np.array([0,0,-0.25,1])
                back_shifted_goal =   (T_torus_world@back_shifted_goal)[:3].tolist()
                x_obs2,y_obs2,z_obs2 = back_shifted_goal[0],back_shifted_goal[1],back_shifted_goal[2]
            

                x_rob,y_rob,z_rob = x_var[0,i],x_var[2,i],x_var[4,i]
            
                
                distance_center = cs.sqrt((x_obss - x_rob)**2 + (y_obss - y_rob)**2 + (z_obss - z_rob)**2) 
                distance_center2 = cs.sqrt((x_obs2 - x_rob)**2 + (y_obs2 - y_rob)**2 + (z_obs2- z_rob)**2) 


                coefficient_att = 0.2
                coefficient_att2 = 0.05
                coefficient_rep = 0.01
                for ii in range(pointsOntorus_world.shape[1]):
                    closest_point = pointsOntorus_world[:,ii]
                
                    distance_surface = cs.sqrt((closest_point[0] - x_rob)**2 + (closest_point[1] - y_rob)**2 + (closest_point[2] - z_rob)**2) 
                    cost+=  coefficient_rep / distance_surface
  
                if index == goal_obs_indx:
                    if i==0:
                        p.addUserDebugLine([x_obss, y_obss, z_obss], [self.env.state[0], self.env.state[2], self.env.state[4]], [0, 1, 0],physicsClientId=self.env.PYB_CLIENT)
                        p.addUserDebugLine([x_obs2, y_obs2, z_obs2], [self.env.state[0], self.env.state[2], self.env.state[4]], [0, 1, 0],physicsClientId=self.env.PYB_CLIENT)
                        p.addUserDebugLine(self.env.state[[0,2,4]],closest_point , [0, 1, 1],physicsClientId=self.env.PYB_CLIENT)
                    cost += coefficient_att * (distance_center)  
                    cost += coefficient_att2 * (distance_center2) 
                else:
                    coefficient_att=0
        for i in range(self.T):
        ...
        for key in keys_obstac:
            ...
            d_robot = obstac["d_robot"]
            x_obs,y_obs,z_obs = obstac["x_center"],obstac["y_center"],obstac["z_center"],
            x_rob,y_rob,z_rob = x_var[0,-1],x_var[2,-1],x_var[4,-1]            
            distance = cs.sqrt((x_obs - x_rob)**2 + (y_obs - y_rob)**2 + (z_obs - z_rob)**2)
            min_distance = (d_robot+d_obs)/2
    ...
    def select_action(self,
        ...
        self.cost.append(sol.value(cost))
        self.x_prev = x_val
        self.u_prev = u_val
    ...
    def get_references(self):
        ...
        if self.env.TASK == Task.STABILIZATION:
            ...
            self.startraj_indx = start
            self.endtraj_indx = end
        else:
    ...
    def run(self,
        ...
        if obss:
            ...
            obs,info = self.initial_obs, self.initial_info 
            self.reset_results_dict()
        ...
        ep_returns, ep_lengths =self.ep_returns, self.ep_lengths
        ...
        self.done = False
        common_metric = 0
        while not(self.done and terminate_run_on_done) and i < self.T and not (self.terminate_loop):
            ...
            self.results_dict['done'].append(self.done)
            self.results_dict['info'].append(info)
            self.results_dict['action'].append(action)
            self.results_dict['state'].append(env.state)
            self.results_dict['state_mse'].append(info["mse"])
            self.results_dict['state_error'].append(env.state - env.X_GOAL[waarde,:])
            waarde += 1
